    ...
    try {
      ...
      sortedDates.forEach((dateKey, index) => {
        ...
        if (finalY + 60 > doc.internal.pageSize.getHeight()) {
          ...
          if (signatureBase64) {
            doc.addImage(signatureBase64, 'PNG', 15, 25, 80, 48);
          }
          
          doc.text("FIRMAN JAMALUDDIN, S.STP", 20, 65);
          const textWidth = doc.getTextWidth("FIRMAN JAMALUDDIN, S.STP");
          doc.line(20, 66, 20 + textWidth, 66);
          doc.text("19820103 200112 1 003", 20, 70);
          doc.text("Penata TK I - IIId", 20, 75);
          doc.setFont("helvetica", "normal");
        } else {
          doc.setFont("helvetica", "bold");
          ...
          if (signatureBase64) {
            doc.addImage(signatureBase64, 'PNG', 15, finalY + 5, 80, 48);
          }
          
          doc.text("FIRMAN JAMALUDDIN, S.STP", 20, finalY + 45);
          const textWidth = doc.getTextWidth("FIRMAN JAMALUDDIN, S.STP");
          doc.line(20, finalY + 46, 20 + textWidth, finalY + 46);
          doc.text("19820103 200112 1 003", 20, finalY + 50);
          doc.text("Penata TK I - IIId", 20, finalY + 55);
          doc.setFont("helvetica", "normal");
        }
      });
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      sortedDates.forEach((dateKey, index) => {
        ...
        // Title
        wsData.push(['RENCANA KEGIATAN KECAMATAN UJUNG PANDANG']);
        ...
        // Metadata
        wsData.push(['HARI', `: ${getIndoDay(dateKey)}`]);
        currentRow += 1;
        wsData.push(['TANGGAL', `: ${getIndoMonth(dateKey)}`]);
        currentRow += 1;
        ...
        // Table Header
        wsData.push(['NO.', 'WAKTU', 'KEGIATAN', 'TEMPAT/LOKASI', 'PAKAIAN', 'PEJABAT PENDAMPING/PEJABAT YANG MEWAKILI', 'PEJABAT', 'KETERANGAN', 'HUMAS']);
        currentRow += 1;
        
        // Table Data
        groupedData[dateKey].forEach((item, i) => {
          wsData.push([
            i + 1,
            item.time ? item.time.split(' ')[0] : '-',
            item.title || '-',
        ...
        wsData.push(['', '', '', '', '', '', '', 'SEKRETARIS CAMAT']);
        wsData.push([]);
        wsData.push([]);
        wsData.push([]);
        wsData.push(['', '', '', '', '', '', '', 'FIRMAN JAMALUDDIN, S.STP']);
        wsData.push(['', '', '', '', '', '', '', '19820103 200112 1 003']);
        wsData.push(['', '', '', '', '', '', '', 'Penata TK I - IIId']);
        currentRow += 10;
      });
      ...
      ws['!cols'] = [
        ...
        { wch: 10 }, // WAKTU
        { wch: 40 }, // KEGIATAN
        { wch: 25 }, // TEMPAT/LOKASI
        { wch: 15 }, // PAKAIAN
        { wch: 40 }, // PEJABAT PENDAMPING
        { wch: 15 }, // PEJABAT
        { wch: 20 }, // KETERANGAN
        { wch: 10 }  // HUMAS
      ];
      ...
      const lastDate = sortedDates[sortedDates.length - 1];
      const fileName = firstDate === lastDate 
        ? `Laporan_Agenda_${firstDate}.xlsx`
        : `Laporan_Agenda_${firstDate}_sd_${lastDate}.xlsx`;
      ...
      <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm space-y-8">
        ...
        <div className="pt-4 border-t border-gray-50 flex flex-col sm:flex-row items-center gap-4">
          ...
          <button 
            ...
            disabled={isGenerating}
            className={`flex-1 w-full sm:w-auto bg-emerald-600 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-emerald-600/20 flex items-center justify-center gap-3 transition-all active:scale-[0.98] ${isGenerating ? 'opacity-70 cursor-not-allowed' : 'hover:bg-emerald-700'}`}
          >
            {isGenerating ? (
              <>
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
